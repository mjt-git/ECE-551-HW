...
board_t * makeBoard(int w, int h, int numMines) {
  //WRITE ME!
  board_t * board1 = malloc(sizeof(*board1));
  board1->width = w;
  board1->height = h;
  board1->totalMines = numMines;
  board1->board = malloc(h * sizeof(*(board1->board)));
  // initialize board1
  for (int i = 0; i < h; i++) {
    board1->board[i] = malloc(w * sizeof(*board1->board[i]));
    for (int j = 0; j < w; j++) {
      board1->board[i][j] = UNKNOWN;
    }
  }
  // randomly put mines
  for (int i = 0; i < numMines; i++) {
    addRandomMine(board1);
  }
  return board1;
}
...
int isInBoard(board_t * b, int x, int y) {
  int w = b->width;
  int h = b->height;
...
int countMines(board_t * b, int x, int y) {
  ...
  for (int i = x - 1; i <= x + 1; i++) {
    for (int j = y - 1; j <= y + 1; j++) {
      ...
      flag = isInBoard(b, i, j);
      if (flag == 1 && IS_MINE(b->board[j][i])) {
        count++;
      }
    }
  }
